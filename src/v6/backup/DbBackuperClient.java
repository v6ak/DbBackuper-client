    ...
    if( args.length == 0 ){
      ...
      try{
        for (int i = 0; i < args.length; i++) {
            ...
            if( url != null ){
              throw new ArgException("Duplicitni -url na pozici "+(i+1));
            };
            try{
              url = args[i+1];
              i++;
            }catch(ArrayIndexOutOfBoundsException e){
              throw new ArgException("Ocekavana url, nalezen konec");
            };
          }else if( args[i].equals("-pwd") ){
            if( pwd != null ){
              throw new ArgException("Duplicitni -pwd na pozici "+(i+1));
            };
            try{
              pwd = args[i+1];
              i++;
            }catch(ArrayIndexOutOfBoundsException e){
              throw new ArgException("Ocekavano heslo, nalezen konec");
            };
          }else if( args[i].equals("-nodata") ){
            if( !data ){
              throw new ArgException("Duplicitni -nodata na pozici "+(i+1));
            };
            data = false;
            ...
            throw new ArgException("Neznamy argument na pozici "+(i+1)+": "+args[i]);
          };
        };
        if( url == null ){
        ...
        if( pwd == null ){
          ...
          if( c == null ){
            throw new ArgException("Argument -pwd pri vypnutem konzolovem vstupu vyzadovan!");
          };
          System.out.print("Zadejte heslo: ");
          pwd = new String(c.readPassword());
          System.out.println();
        };
        new Thread(new DbBackuper(url, pwd).setBackupTableData(data).setBackupTableDefinition(def)).start();
