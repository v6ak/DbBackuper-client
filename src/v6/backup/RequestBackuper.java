...
abstract class RequestBackuper implements Runnable{
  ...
  public void run(){
    ...
    try{
    	...
    	@Cleanup("disconnect")
    	HttpURLConnection conn = (HttpURLConnection)new URL(urlBase + getUrlSuffix()).openConnection();
    	conn.setDoInput(true);
    	conn.setDoOutput(true);
    	conn.setRequestMethod("POST");
    	dbb.connectionModifer.modify(conn);
    	in = new BufferedReader(new InputStreamReader(conn.getInputStream()));
      out = new PrintWriter(new BufferedWriter(new FileWriter(dbb.filePrefix+getFileName())));//*/System.out;
      String s, lastline="";
